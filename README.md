...
4. Use **Open Full Monitoring** for the larger timeline. **Diagnose My Machine** copies a private, minimized context brief only when you click it. Nothing is uploaded or sent for you.
...
- Clicking the menu-bar icon opens a centered, cached view of the last hour immediately, refreshes that view from the local database each time it opens, and provides a visible one-click refresh; the full Monitoring window and a dedicated expanded timeline remain one click away
- System, Light, and Dark appearances apply consistently to the menu, main app, and expanded timeline. The expanded timeline is a native resizable window with one-click macOS full screen; it keeps the graph dominant and places additional interpretation behind a disclosure.
